    ...
    <div class="twelve columns">
        <section class="vertical tabs">
            ...
            <div class="tab-content nine columns">
                ...
                <p>プロビジョニングシェルを流して生成される、デフォルトの環境を使用する場合であれば、Linodeをおすすめします。同じスペックでも他のクラウドサービスより、内部の処理スピードが早いでし、東京にサーバーがあります。アウトの出力回線帯幅の制限がやや細いですが、短期のチュートリアルでは問題にならないでしょう。</p>
                <p>チュートリアル／ハンズオンを数日などに渡り開催する場合は、AWSをおすすめします。大手だけあり、DDos攻撃などは上部の回線で対処してくれるようです。</p>
                <p>コードを編集し、結果を確認するチュートリアルであれば、さほど能力を上げる必要はないと思われます。もし、Composerを使って外部パッケージを追加するようなハンズオンを提供するのであれば、かなりメモリも食いますので、スペックは高くしておく必要があるでしょう。</p>
                <h5>デフォルト環境</h5>
               ...
               <p>デフォルト環境は、Codiadの機能を元に構築しているため、セキュリティー的に完璧ではありません。「チュートリアルは無料で数時間のものであるので、参加者が悪事を働いたり、外部から攻撃を受け継続できない場合は、中止でかまわないだろう。」という考えです。もちろん、参加希望者には事前に了承してもらいましょう。</p>
                ...
                <p>後は起動したUbuntu14.04の環境でルートユーザーになり、シェルで流してください。</p>
                <pre><code>base -v ubuntu-provisioning.sh</code></pre>
                ...
                <p>"Jaild-Terminal"は、オリジナルの"Terminal"を改良したものです。cdコマンドによる移動の制限、使用コマンドのホワイトリスト指定、エイリアスの登録機能などを追加しています。（エイリアスは現在バグっています。）また、UIを日本語に変更しています。このプラグインは、公式ではないためインストールするには、マーケットプレースの一番下にあるテキストエリアに"https://github.com/InstantLaravel/Codiad-Jailed-Terminal.git"を指定し、右横の"Install Manually"ボタンをクリックしてください。</p>
                ...
                <p>ユーザー周りの管理が多少ややこしいのですが、結果を確認するためのWebアクセスは、PHP-FPMの指定で各ユーザー権限としてアクセスさせるためです。suExeをNginxで動作させる方法を探るほうが、簡単になるかも知れません。今回は、suExeは遅いので、できるのだとしてもPHP−FPMで頑張ってみました。</p>
                <p>名前付けと管理が簡単のため、ポート分けではなく、ソケットファイル名を利用しています。</p>
                <p>PHP-FPMでユーザーごとの動作環境を用意するということは、ユーザー分のメモリを必要とするということです。</p>
                <p>確認時のPHP−FPMを同一動作ユーザーで行うと、他の参加者の環境へPHPから簡単にアクセスできてしまうため、最低限度のセキュリティーとして動作ユーザーを分けることが必要となりました。それでも完璧ではないでしょう。そのため、現在のWeb IDE環境サービスは、各プロジェクトごとにVPSを建てるのが主流なのでしょう。近い将来はDockerになるでしょうが、Dockerは今のところセキュリティー面が弱いらしく、それを強化するのがノウハウとしてビジネスになっているようです。</p>
            </div>
        </section>
    </div>
    @stop
